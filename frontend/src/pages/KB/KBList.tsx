  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-[#1A2222] rounded-xl border border-neutral-200 dark:border-neutral-700 p-3 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">Total</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#1A2222] rounded-xl border border-neutral-200 dark:border-neutral-700 p-3 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">Publicados</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#1A2222] rounded-xl border border-neutral-200 dark:border-neutral-700 p-3 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">Visualizacoes</span>
          </div>
        ...
        <div className="bg-white dark:bg-[#1A2222] rounded-xl border border-neutral-200 dark:border-neutral-700 p-3 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase">Feedbacks Uteis</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar por titulo ou conteudo..."
              ...
              className="w-full pl-9 pr-3 py-2 text-sm border border-neutral-200 dark:border-neutral-700 rounded-lg focus:ring-1 focus:ring-emerald-500 outline-none" />
          </div>
          <div className="relative">
            <Filter size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
            <select value={categoriaFiltro} onChange={(e) => setCategoriaFiltro(e.target.value)}
              className="w-full pl-8 pr-3 py-2 text-sm border border-neutral-200 dark:border-neutral-700 rounded-lg focus:ring-1 focus:ring-emerald-500 outline-none appearance-none bg-white dark:bg-[#1A2222]">
            ...
            {(['todos', 'publicados', 'rascunhos'] as const).map((opt) => (
              <button key={opt} onClick={() => setPublicadoFiltro(opt)}
                className={`flex-1 px-3 py-1.5 text-xs font-semibold rounded-md transition-colors capitalize ${
                  publicadoFiltro === opt ? 'bg-emerald-100 text-emerald-700' : 'text-neutral-600 dark:text-neutral-300 hover:bg-neutral-100 dark:bg-neutral-800'
                }`}>
                {opt}
              </button>
            ))}
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="bg-white dark:bg-[#1A2222] rounded-xl border border-neutral-200 dark:border-neutral-700 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-neutral-50 dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-700 text-xs text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
            ...
            <tbody>
              {items.map((art) => (
                <tr key={art.id} className="border-b border-neutral-100 dark:border-neutral-800 hover:bg-neutral-50 dark:bg-neutral-900">
                  <td className="py-3 px-4">
                    <div className="font-semibold text-navy-900">{art.titulo}</div>
                    {art.resumo && <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5 line-clamp-1">{art.resumo}</p>}
                    {art.tags && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {art.tags.split(',').filter(Boolean).slice(0, 3).map((t) => (
                          <span key={t} className="text-[10px] bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 px-1.5 py-0.5 rounded">
                            {t.trim()}
                          </span>
                        ))}
                      </div>
                  ...
                  <td className="py-3 px-4 hidden md:table-cell">
                    {art.categoria ? (
                      <span className="text-xs font-medium text-neutral-700 dark:text-neutral-200">{art.categoria.nome}</span>
                    ) : (
                  ...
                  <td className="py-3 px-4 text-center">
                    {art.publicado ? (
                      ...
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 inline-flex items-center gap-1">
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => verArtigo(art)} className="p-1.5 rounded hover:bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300" title="Visualizar">
                        <Eye size={14} />
                      </button>
                      {canEdit(user?.role) && (
      ...
      {showViewer && artigoAtual && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setShowViewer(false)}>
          <div className="bg-white dark:bg-[#1A2222] rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white dark:bg-[#1A2222] border-b border-neutral-200 dark:border-neutral-700 px-6 py-4 flex items-start justify-between">
              ...
              <button onClick={() => setShowViewer(false)} className="text-neutral-400 hover:text-neutral-600 dark:text-neutral-300 p-1 flex-shrink-0">
                <XCircle size={20} />
              </button>
            </div>
            <div className="p-6">
              {artigoAtual.resumo && (
                <p className="text-sm text-neutral-600 dark:text-neutral-300 italic border-l-2 border-emerald-300 pl-3 mb-4">
                  {artigoAtual.resumo}
                </p>
              )}
            ...
            <div className="sticky bottom-0 bg-white dark:bg-[#1A2222] border-t border-neutral-200 dark:border-neutral-700 px-6 py-3 flex items-center justify-between">
              <p className="text-xs text-neutral-500 dark:text-neutral-400">Este artigo foi util?</p>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setShowViewer(false)} className="text-xs text-neutral-500 dark:text-neutral-400 hover:text-neutral-700 dark:text-neutral-200 ml-2">
                  Fechar
                </button>
              </div>
            </div>
