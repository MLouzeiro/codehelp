  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Layers className="text-blue-600" size={24} /> Niveis de Suporte
          </h1>
      ...
      <div className="space-y-2">
        {niveis.length === 0 && (
          <p className="text-center text-neutral-400 py-8 text-sm">Nenhum nivel cadastrado.</p>
        )}
        {niveis.map((nivel) => (
          <div key={nivel.id} className={`card flex items-center gap-3 p-3 ${!nivel.ativo ? 'opacity-60' : ''}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold text-sm text-gray-900 truncate">{nivel.nome}</h3>
                <code className="text-[10px] text-neutral-500 bg-neutral-100 px-1.5 py-0.5 rounded font-mono">{nivel.slug}</code>
                {nivel.slaMinutos != null && (
                  <span className="text-[10px] text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded font-medium">SLA {nivel.slaMinutos}min</span>
                )}
                {!nivel.ativo && (
                  <span className="text-[10px] text-neutral-500 bg-neutral-100 px-1.5 py-0.5 rounded font-medium uppercase">Inativo</span>
                )}
              </div>
              {nivel.descricao && <p className="text-xs text-neutral-500 truncate mt-0.5">{nivel.descricao}</p>}
            ...
            {isMaster && (
              <div className="flex items-center gap-1">
                <button onClick={() => { setEditing(nivel); setCreating(false); }} className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600" title="Editar">
                  <Edit2 size={14} />
                </button>
                <button onClick={() => handleToggle(nivel)} className="p-1.5 hover:bg-neutral-100 rounded text-neutral-600" title={nivel.ativo ? 'Desativar' : 'Reativar'}>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-xl p-5 w-full max-w-lg space-y-3" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-gray-900">{isNew ? 'Novo Nivel' : `Editar "${nivel!.nome}"`}</h3>
