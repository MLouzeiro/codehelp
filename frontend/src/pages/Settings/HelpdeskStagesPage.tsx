  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {stages.length === 0 && (
          <p className="text-center text-neutral-400 py-8 text-sm">Nenhuma etapa cadastrada.</p>
        )}
        {stages.map((stage) => (
          <div
            key={stage.id}
          ...
          >
            {isMaster && <GripVertical size={14} className="text-neutral-400 cursor-grab" />}
          ...
          <div className="mt-1.5 flex flex-wrap gap-1">
            {VARIAVEIS.map((v) => (
              <button key={v} type="button" onClick={() => setForm({ ...form, autoMessage: (form.autoMessage || '') + v })}
                className="text-[10px] text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded font-mono hover:bg-blue-100">
                {v}
              </button>
            ))}
          </div>
